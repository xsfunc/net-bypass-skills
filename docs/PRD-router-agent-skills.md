...
Each skill is self-contained (inlines the operational safety procedures: safe-mode snapshot/rollback timer, PM-axis detection, preflight, validation, audit logging — except `dpi-tspu-strategy-theory`, a theory skill with no router-facing steps, exempt per ADR-0005), evidence-tagged (`verified` / `community-observed` / `hypothesis`), and uses a pack structure (SKILL.md index + separate reference files) for all four skills. Out-of-scope content is explicitly excluded and must not trigger these skills.
...
28. As a router-agent operator, I want both `zapret2-router-deploy` and `dpi-tspu-strategy-theory` to use a pack structure (index + reference files), so that the larger deploy skill loads only the needed reference (deploy/nfqueue-wiring/theory/hostlist/blockcheck) and the DPI-theory skill isolates its hypothesis-tier material in a separate gated reference (`hypothesis-frontier`) so activation stays lean and speculation is loaded only on demand. *(Amended for #5: `router-deploy` upgraded from single-file to pack. Amended for #6: `dpi-tspu-strategy-theory` likewise upgraded from single-file to pack — see Implementation Decisions and ADR-0003.)*
...
- **Structure:** pack (SKILL.md index + reference files) for all four skills — `zapret2-strategies`, `zapret2-engine-reference`, `zapret2-router-deploy`, and `dpi-tspu-strategy-theory`. (`zapret2-router-deploy` was originally specified as single-file; amended to pack after the #5 grilling — distilling ~1700 source lines into a single SKILL.md would bloat context loading on every activation. Pack: SKILL.md index + 5 reference files — `deploy`, `nfqueue-wiring`, `theory`, `hostlist-ipset-nftset`, `blockcheck`. `dpi-tspu-strategy-theory` was likewise originally single-file; amended to pack after the #6 grilling — the epistemic seam between verified+community-observed core and the hypothesis tier is the pack boundary: SKILL.md index + 4 reference files — `siberian-scheme`, `ja3-ja4`, `ttl-detection`, `hypothesis-frontier` (gated). See ADR-0003.)
...
- **Source-file mapping per skill:**
  ...
  - `dpi-tspu-strategy-theory`: `DPI/{dpi-analysis-pipeline,browser-ja4-fingerprint-block,tspu-false-blocks-june-2026,tspu-h2-h3-fingerprint-hypothesis,statistical-morphing-concept}.md` and the detection-model / JA3-JA4 portion only of `VLESS/dpi-tls-june-2026.md`. (TTL detection vector sourced from `DPI/dpi-analysis-pipeline.md` Stage 0, *not* `VLESS/dpi-tls-june-2026.md` — the latter has no TTL content; flag mechanics stay in `zapret2-engine-reference/fooling.md`. See ADR-0001.)
...
- **`VLESS/dpi-tls-june-2026.md`:** large and mostly VLESS-transport; only the detection-model / JA3-JA4 / Siberian-scheme semantics are extracted into `dpi-tspu-strategy-theory` (the TTL detection vector is sourced from `DPI/dpi-analysis-pipeline.md` Stage 0 instead — see ADR-0001), the VLESS/REALITY/XHTTP transport engineering is discarded.

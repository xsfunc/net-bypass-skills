...
28. As a router-agent operator, I want `zapret2-router-deploy` to use a pack structure (index + reference files) and `dpi-tspu-strategy-theory` as a single SKILL.md file, so that the larger deploy skill loads only the needed reference (deploy/nfqueue-wiring/theory/hostlist/blockcheck) while the smaller DPI-theory skill stays simple to maintain and load in one shot. *(Amended for #5: originally both were single-file; `router-deploy` upgraded to pack — see Implementation Decisions.)*
...
- **Structure:** pack (SKILL.md index + reference files) for `zapret2-strategies`, `zapret2-engine-reference`, and `zapret2-router-deploy`; single SKILL.md for `dpi-tspu-strategy-theory`. (`zapret2-router-deploy` was originally specified as single-file; amended to pack after the #5 grilling — distilling ~1700 source lines into a single SKILL.md would bloat context loading on every activation. Pack: SKILL.md index + 5 reference files — `deploy`, `nfqueue-wiring`, `theory`, `hostlist-ipset-nftset`, `blockcheck`.)
